	...
	view = {
		...
		cancelPreSelectedNode: function (setting, node) {
			...
			for (var i=0, j=list.length-1; j>=i; j--) {
				if (!node || node === list[j]) {
					//coral2013.12.23 选中样式修改
					if(setting.view.isMenu==true){
						$("#" + list[j].tId ).removeClass(consts.node.CURSELECTED);
					}
					else{
		...
		selectNode: function(setting, node, addFlag) {
			...
			//coral2013.12.23 选中样式修改
			if(setting.view.isMenu==true){
				$("#" + node.tId ).addClass(consts.node.CURSELECTED);
			}
			else{
			...
			function _docMouseMove(event) {
				...
				if (root.dragFlag == 0) {
					...
					for (i=0, l=nodes.length; i<l; i++) {
						...
						//coral2013.12.23 选中样式修改
						if(setting.view.isMenu==true){
							tmpDom.removeClass(consts.node.CURSELECTED);
						}
						else{
